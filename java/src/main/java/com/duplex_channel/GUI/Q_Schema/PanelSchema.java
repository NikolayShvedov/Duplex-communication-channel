package com.duplex_channel.GUI.Q_Schema;

import com.duplex_channel.GUI.GUIStatistic.GUIStatistic;
import com.duplex_channel.GUI.Utils.Components.*;
import com.duplex_channel.GUI.Utils.Components.Package;
import com.duplex_channel.GUI.Utils.Components.Point;
...
public class PanelSchema extends JPanel implements Runnable {
    ...
    public void run() {
        ...
        for (int i = 0; i < Utils.RUN_COUNT; i++) {
            ...
            instance.addPackageCountA(transactCountA);
            instance.addPackageCountB(transactCountB);
            instance.addDonePackages(doneTransactCount);
            instance.addDeclinePackageCount(declineCount);
            instance.addCommunicationСhannel(channelAB);
            instance.addCommunicationСhannel(channelBA);
            instance.addCommunicationСhannel(sputnikLine);
            new GUIStatistic(simulationNumber, instance);
    ...
    void reStatistic() {
        channelAB.setPackage_count(0);
        channelAB.setStatistic_workTime(0);
        channelAB.setWorkTime(0);
        channelBA.setPackage_count(0);
        channelBA.setStatistic_workTime(0);
        channelBA.setWorkTime(0);
        sputnikLine.setPackage_count(0);
        sputnikLine.setStatistic_workTime(0);
        sputnikLine.setWorkTime(0);
        transactCountA = 0;
        transactCountB = 0;
        doneTransactCount = 0;
        declineCount = 0;
